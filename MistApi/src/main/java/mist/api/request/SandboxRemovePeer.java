...
class SandboxRemovePeer {
    static int request(byte[] id, Peer peer, Sandbox.RemovePeerCb callback) {
        final String op = "sandbox.removePeer";
        ...
        return MistApi.getInstance().request(buffer.toByteArray(), new MistApi.RequestCb() {
            ...
            private MistApi.RequestCb init(Sandbox.RemovePeerCb callback) {
                this.cb = callback;
                return this;
            }
        }.init(callback));
